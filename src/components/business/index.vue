<template>
  <div id="business">
        ...
        <el-col :span="8">
          ...
          <div style="display: inline-block;">
            <div style="display: flex;justify-content: space-between;flex-wrap: nowrap;" class="font_style">
              <el-checkbox label="全部" v-model="helpParams.composeShow" style="margin-right: 30px;"
                           @change="composeAllCheck"></el-checkbox>
              <el-checkbox-group v-model="helpParams.compose" @change="handleChangeCompose">
                <el-checkbox v-for="compose in helpParams.composeList" :label="compose.name"
                             :key="compose.value"></el-checkbox>
              </el-checkbox-group>
            </div>
          </div>
        </el-col>
    ...
    <div class="container">
      <div style="margin-top: 20px; width: 58%;">
        ...
        <el-radio-group v-model="helpParams.time" @change="handleChangeDate">
          ...
          <el-radio label="day">
            <template>
              <el-input @focus="handleFocusDays" type="number" size="mini" style="width: 80px;"
                        v-model="helpParams.days" @change="handleInputDays"></el-input>
            </template>
            天
          </el-radio>
          <el-radio label="date">
      ...
      <div style="margin-top: 20px;width: 42%;text-align:left;" class="composeCheck">
        ...
        <el-select v-model="params.order_field" size="mini" clearable @change="handleOrderField" style="width:20%;">
          <el-option v-for="field in businessFieldList" :label="field.name" :key="field.value"
      ...
      >
          ...
          <el-table-column label="区域" min-width="150px;" prop="group" class-name="column-area">
            <template slot-scope="scope">
              <div>
                ...
                <span v-else></span>
                <el-button v-if="scope.row.first && scope.row.group" type="text" size="mini" icon="el-icon-plus"
                           @click.stop="handleAddClick(scope)">时间段
                </el-button>
              </div>
            </template>
          </el-table-column>
        ...
        <el-table-column label="收房">
          ...
          <el-table-column label="渠道单比例" prop="lord.agency_percentage">
            <template slot-scope="scope">
              <span v-if="scope.row.lord && scope.row.lord.agency_percentage && scope.row.lord.agency_percentage != 0">{{ (parseFloat(scope.row.lord.agency_percentage) * 100).toFixed(1) }}%</span>
            </template>
          </el-table-column>
        ...
        <el-table-column label="租房">
          ...
          <el-table-column label="渠道单比例" prop="renter.agency_percentage" class-name="column-agency">
            <template slot-scope="scope">
              <span
                v-if="scope.row.renter && scope.row.renter.agency_percentage && scope.row.renter.agency_percentage != 0">{{ (parseFloat(scope.row.renter.agency_percentage) * 100).toFixed(1) }}%</span>
            </template>
          </el-table-column>
        ...
        <el-table-column label="空置">
          ...
          <el-table-column label="平均消化时长" prop="vacant.solve_time" class-name="column-day"></el-table-column>
        </el-table-column>
    ...
    <el-dialog
      ...
      <div style="position: absolute;right: 18px;top: 70px;" v-if="activeName === 'third'">{{unexpected}}</div>
...
<script>
  ...
  export default {
        ...
        businessList: [
          {
            ...
            collect: {num: 400, zhongjie: '50%', avg: '3500/月', kongzhi: '20天', months: '20月'},
            rent: {num: 200, kongzhi: '10天', qudao: '50%', avg: '3500/月', back: '20天', avgPrice: '-500'},
            other: {kongzhi: '150套', kongzhiday: '20天', all: 15000}
          },
        ],
    ...
    methods: {
      ...
      //详情
      geUnexpected(params) {
        this.$http.get(this.url + 'performance/business/unexpected', {
          params
        }).then(res => {
          if (res.data.code === '20000') {
            this.unexpected = res.data.data;
          } else {
            this.unexpected = '';
          }
        }).catch(err => {
      ...
      handleCellClick(row, column) {
        ...
        this.geUnexpected(detailParams);
      },
      //添加时间段
