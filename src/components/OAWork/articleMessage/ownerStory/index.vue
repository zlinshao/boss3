...
<script>
  ...
  export default {
    ...
    methods: {
      getDict() {
        this.dictionary(147, 1).then((res) => {
      ...
      // 详情
      openDetail(row) {
        let data = {ids: row.id, detail: 'port'};
      ...
      //置顶
      top(id, info) {
        this.$http.put(this.urls + "oa/portal/status/" + id, {type: 'top'}).then((res) => {
      ...
      //精华
      essence(id, info) {
        this.$http.put(this.urls + "oa/portal/status/" + id, {type: 'fine'}).then((res) => {
          if (res.data.code === "800100" || res.data.code === "800110") {
            this.prompt(1, res.data.msg);
            this.intelligence();
            this.getDict();
