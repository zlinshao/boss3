    ...
    export default {
      ...
      methods: {
        getTableData(){
          ...
          this.isHigh = false;
          this.$http.get(globalConfig.server+'salary/periodic/city?limit=12&page='+this.form.page
        ...
        // 导出
        exportData() {
          this.$http.get(globalConfig.server+'salary/periodic/export/city', { responseType: 'arraybuffer'}).then((res) => { // 处理返回的文件流
            if (!res.data) {
              return;
            }
