<template>
  <div>
    <!--<el-dialog :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" width="0"-->
               <!--style="margin-top:20vh" :visible.sync="badgeDialogVisible">-->
      <!--<div class="badgeup" v-for="(item,index) in infoMore" :key="item.id" v-if="a ==index">-->
        ...
        <!--</div>-->
        <!--<div class="button">-->
          <!--<el-button v-show="a <= len-1 && a>0 " class="elbuttom1" size="small" @click="preimg" type="primary">上一张-->
          <!--</el-button>-->
          <!--<el-button v-show="a == 0" class="elbuttom1" size="small" type="info">上一张</el-button>-->
          <!--<el-button v-show="a < len - 1" class="elbuttom2" size="small" @click="nextimg(item.id)" type="primary">下一张-->
          <!--</el-button>-->
          <!--<el-button v-show="a == len -1" class="elbuttom2" size="small" @click="close(item.id)" type="primary">我知道了-->
          <!--</el-button>-->
        <!--</div>-->
      <!--</div>-->
    <!--</el-dialog>-->
...
<script>
  export default {
    ...
    methods: {
      ...
      //   this.a = this.a + 1;
      //   this.form.id = id;
      //   this.$http.get(globalConfig.server + "oa/portal/know", {params: this.form}).then(res => {
      //   });
      // },
      // close(id) {
      //   this.form.id = id;
