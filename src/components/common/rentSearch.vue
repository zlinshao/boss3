<template>
  <div id="mapSearchId">
    <el-dialog :close-on-click-modal="false" title="房屋选择" :visible.sync="rentDialogVisible" width="50%" :before-close="closeDialog">
      <div class="content">
...
<script>
  ...
  export default {
    ...
    watch:{
      rentDialog(val){
        this.rentDialogVisible = val
      },
      rentDialogVisible(val) {
        if (!val) {
          this.$emit('close');
        }
      }
