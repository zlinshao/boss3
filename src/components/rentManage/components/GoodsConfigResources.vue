<template>
  ...
  <el-table
    ...
    <el-table-column label="单价（元）">
        <template slot-scope="scope">
          <el-input v-model="form.price[scope.$index]" v-on:input="pricechange(scope.$index)" placeholder=""></el-input>
        </template>
    </el-table-column>
    <el-table-column label="数量">
        <template slot-scope="scope">
          <el-input v-model="form.num[scope.$index]" v-on:input="numchange(scope.$index)" placeholder=""></el-input>
        </template>
    </el-table-column>
    ...
    <el-table-column label="操作" width="80px" >
        <template slot-scope="scope" >
          <i @click="deletex(scope.$index)" class="iconfont icon-shibai" style="color: #6a8dfb;font-size:27px;line-height:34px; margin-left:12px;cursor: pointer;"></i>
        </template>
    </el-table-column>
  ...
  <AddGoodsResources :addGoodsDialog="addGoodsDialog" @close="closeAddGoodsResources" @addGoodsOk="addGoodsOk"></AddGoodsResources>
  </div>
</template>
