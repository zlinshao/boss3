<template>
  <div id="rentingDetail"
        ...
        <h3>
          ...
          <el-dialog title="合同公司联录入" :visible.sync="contractEntry" width="40%">
              ...
              <el-row>
                <el-col :span="24">
                  <el-form-item label="是否接通" required>
                    <el-select v-model="contractForm.is_connect" placeholder="请选择">
                      <el-option v-for="item in newconnectCategory" :label="item.name" :key="item.id"
            ...
            <span slot="footer" class="dialog-footer">
              ...
              <el-button type="primary" @click="confirmAgain = true" size="mini">确 定</el-button>
            </span>
          </el-dialog>
          <el-dialog title="提示" :visible.sync="confirmAgain" width="30%">
            <span>确定保存</span>
            <span slot="footer" class="dialog-footer">
          ...
          <el-dialog title="合同对比" :visible.sync="contrastContractDialog" width="30%">
            <div class="content">
              <div style="color: #e4393c;font-size: 14px">客服{{this.contrastContent}}</div>
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="已收金额（汇款账户）">
                  <div class="content">
                     <span v-for="(item,index) in contractInfo.money_table" style="display: block;">
                      {{item.money_sep}}元，{{item.money_way}} <span
          ...
          <el-col :span="12" id="simple" class="simple_left scroll_bar"
            ...
            <div class="contractInfo">
              <el-form size="small" label-width="180px" label-position="right" style="margin-left: 100px;">
                ...
                <el-form-item label="月单价(元)" class="blueColor">
                  <div class="content">
                    <span v-for="(item,index) in contractInfo.price">
                      {{item.price}}元，{{item.period}}个月 <span v-show="index<contractInfo.price-1">;</span>
                    </span>
                ...
                <el-form-item label="是否中介" class="blueColor">
                  <div class="content" v-if="contractInfo.is_agency">中介</div>
                  <div class="content" v-else="">个人</div>
                </el-form-item>
                <el-form-item label="原中介费(元)">
    ...
    <MemorandumRecord :memoDialog="memoDialog" is_rent="1" :selectContractId="contract_id"
    ...
    data() {
      return {
        ...
        contractForm: {
          ...
          pay_method: [[],[],],      //支付方式
          has_pay: "",        //已支付的费用
          contract_id: this.$route.query.id,    //合同编号
          module: 2
        },
...
//      this.historyId = document.getElementById('historyId').offsetTop -201;
    ...
    methods: {
      ...
      getContract() {
        ...
        this.$http.get(globalConfig.server + 'contract/contract_diff/detail?module=2&contract_id=' + this.$route.query.id).then(res => {
      ...
      // 对比
      contrastContract() {
        this.$http.get(globalConfig.server + "contract/contract_diff/diff?module=2&contract_id=" + this.$route.query.id).then(res => {
          if(res.data.code == "20010") {
