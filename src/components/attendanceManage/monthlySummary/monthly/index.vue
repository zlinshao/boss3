  ...
  <div id="monthlySummary">
    ...
    <div class="topShow">
      ...
      <el-checkbox-group v-model="checkList" v-show="this.collapse == '收起'">
        <el-checkbox :label="item.name" v-for="(item, index) in this.celeckList" :key="index"
                     @change="selecked(item, index)" :disabled="item.disabled" :checked="item.disabled"></el-checkbox>
      </el-checkbox-group>
    </div>
    ...
    <el-dialog title="导出列表框" :visible.sync="lookListDialog" width="50%">
      <el-table :data="exportListData" border style="width: 100%">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="primary" @click="download(scope.row)" size="mini"
                       :disabled="scope.row.file_id ? false : true">下载
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div class="dialogPage block pages">
        ...
        <el-pagination @size-change="dialogHandleSizeChange" @current-change="dialogHandleCurrentChange"
                       :current-page="params.page" :page-sizes="[12,24, 36,48]" :page-size="params.limit"
    ...
    <div class="table">
      ...
      <el-table :data="tableData" border ref="crayTable">
        <el-table-column :prop="showItem.prop" :label="showItem.name" v-for="(showItem, index) in this.seleckedList"
                         :key="index">
          ...
          <el-table-column v-if="showItem.name == '请假'" v-for="(day, ind) in secondaryMenu" :key="ind"
                           :label="day.label" :prop="day.prop"></el-table-column>
    ...
    <div class="block pages">
      ...
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="params.page"
                     :page-sizes="[12,24, 36,48]" :page-size="params.limit"
                     layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
    </div>
    <!-- 组织架构 -->
    <organization :organizationDialog="organizationDialog" :length="length" :type="type" @close='closeModal'
                  @selectMember="selectMember"></organization>
    <!-- 查看当月考勤 -->
    <el-dialog :title="attendanceName+ params.arrange_month +'的考勤记录'" :visible.sync="attendanceMonth" width="50%"
               :before-close="attendanceDialog">
...
<script>
  ...
  export default {
    ...
    created() {
      this.params.arrange_month = this.getCurrentDate('now');
